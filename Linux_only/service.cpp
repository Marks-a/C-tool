/*
    ...
    [May use daemon for this service (Requirment: manually)]
...
bool is_same_sub(const std::string &sender_ip_str, const std::vector<LocalInfo> &self);
...
void receive_hello(bool &debug, int sock, std::vector<Neighbor> &neighbors, const std::vector<LocalInfo> &self, time_t &now)
{
    ...
    {
        if (!iface.netmask.empty() && is_same_sub(sender_ip_str, self))
        {
            in_same_net = true;
            break;
        }
    ...
    // Update or insert neighbor (Maybe bad cuz multiple func has time func )
